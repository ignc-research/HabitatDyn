...
parser.add_argument('--data_path', metavar='DIR', help='path to parsed .npy files')
...
data_path = '/home/gao/dev/project_remote/gao_dis_estimation_v2/dis_estimation_v2/tmp2/new_eval_251/habitat_sim_excl_static_30scenes_newPitch_originalModel/use_pre/'
...
read_dist = lambda x : np.array([x['pre_location'], x['gt_location'],
...
for record_name in file_list:
    ...
    if record_all.shape[0] == 0:
        ...
        record_all = np.concatenate((record_all,np.array(list(map(read_dist, record)))), axis=0)

print("prepare data...")
record_all_tmp = record_all
index = np.where((record_all_tmp[:,2,1] > 3))
record_all_tmp = record_all_tmp[index]
index = np.where((record_all_tmp[:,2,1] <= 4))
record_all_tmp = record_all_tmp[index]
# detction chance
successful_detection_times = np.sum(np.where(record_all_tmp[:,0,0] != None,1,0) )
...
filtered_detected_frame = np.array([x[:2] for x in detected_frame if x[0,0] != None])
x_sqr = (filtered_detected_frame[:,0,0] - filtered_detected_frame[:,1,0])**2
y_sqr = (filtered_detected_frame[:,0,1] - filtered_detected_frame[:,1,1])**2
